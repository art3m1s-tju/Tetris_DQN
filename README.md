...
```
Tetris_DQN/
├── train.py                  # 主训练脚本（探索期贯穿全程，num_decay_epochs=60000）
├── test.py                   # 推理 / 录像脚本
├── demo_final.mp4            # 主实验最终演示视频
├── src/
...
│   ├── tetris_50000
│   └── tetris_60000          # 最终模型
├── tensorboard/              # 主实验 TensorBoard 训练日志
└── ablation/                 # 消融实验：提前结束探索期
    ├── train.py              # 消融训练脚本（num_decay_epochs=48000）
    ├── test.py               # 消融推理 / 录像脚本
    ├── demo.mp4              # 消融实验演示视频
    ├── src/
    │   ├── deep_q_network.py
    │   └── tetris.py
    ├── trained_models/       # 消融实验模型检查点
└── tensorboard/          # 消融实验 TensorBoard 日志
```
...
主实验将 ε 衰减周期设置为与总训练轮数相同（`num_decay_epochs=60000`），使探索率在整个训练过程中持续缓慢下降。本消融实验将衰减周期缩短为 `num_decay_epochs=48000`，即在第 48,000 轮时 ε 已降至最低值 0.01，剩余 12,000 轮完全以贪心策略运行，以验证探索期提前结束是否会引发灾难性遗忘。

### 实验设置

| 参数 | 主实验 | 消融实验 |
|------|--------|----------|
| `num_decay_epochs` | 60,000 | **48,000** |
| `num_epochs` | 60,000 | 60,000 |
| 其余超参数 | — | 与主实验完全相同 |

### 灾难性遗忘的成因分析

当探索率提前锁定在 0.01 后，训练进入纯贪心阶段，经验回放缓冲区会被大量同质化的高置信度经验迅速填满，导致：

1. **数据分布坍缩**：缓冲区中的样本多样性急剧下降，网络只见到少数重复的"优质"状态，对罕见或困难局面的泛化能力退化。
2. **Q 值过估计加剧**：缺乏随机探索的校正，目标 Q 值持续被高估，损失函数的梯度信号失真。
3. **策略崩溃**：网络在后期训练中对已学会的策略产生过拟合，遇到轻微扰动便出现性能骤降，即灾难性遗忘的典型表现。

### 实验结果

消融实验的 TensorBoard 曲线显示，Score 和 Lines Cleared 在约第 48,000 轮后出现明显下滑或剧烈震荡，而主实验的曲线在同阶段仍保持平稳上升，印证了持续探索对稳定训练的关键作用。
...
> 消融实验的模型检查点保存于 `ablation/trained_models/`，日志保存于 `ablation/tensorboard/`，与主实验完全隔离。
