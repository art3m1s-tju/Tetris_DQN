...
class Tetris:
    ...
    def get_state_properties(self, board):
        ...
        bumpiness, total_height, col_heights, max_height = self.get_bumpiness_and_height(board)

        current_one_hot = [0] * 7
        current_one_hot[self.ind] = 1
        next_one_hot = [0] * 7
        if len(self.bag) > 0:
            next_one_hot[self.bag[-1]] = 1

        # 【关键修改】：
        # 1. 移除了所有归一化操作（/4.0, /20.0 等）。
        # 2. 保留了你想要的 29 维特征。
        # 3. 神经网络会自动通过权重适应这些数值的大小。
        return torch.FloatTensor(
            [lines_cleared, holes, bumpiness, total_height, max_height]
            + col_heights.tolist() 
            + current_one_hot
            + next_one_hot
        )
    ...
    def get_bumpiness_and_height(self, board):
        ...
        return total_bumpiness, total_height, heights, max_height
    ...
    def step(self, action, render=True, video=None):
        ...
        # 【关键修改：奖励函数】
        # 1. 基础生存分：只要能放下去就是 1 分（防止摆烂）。
        # 2. 消行奖励：平方级增长，鼓励多行消除。
        reward = 1 + (lines_cleared ** 2) * self.width
        
        # 3. 惩罚项：只有当空洞变多时才重罚。
        if new_holes > old_holes:
            reward -= 0.1  # 增加空洞是非常不好的行为
            
        if self.gameover:
            reward -= 2

        # 这里的 score 仅用于 UI 显示和 Tensorboard 统计，保持原样即可
